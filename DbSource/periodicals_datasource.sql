...
CREATE TABLE IF NOT EXISTS `periodicals_db`.`subscription` (
  ...
  INDEX `fk_subscription_periodical_idx` (`periodical_title` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_subscription_periodical`
    FOREIGN KEY (`periodical_title`)
    REFERENCES `periodicals_db`.`periodical` (`title`)
    ON DELETE NO ACTION
    ON UPDATE CASCADE)
ENGINE = InnoDB;
